...
from models.aux.nets import *
from modules.federated import ServerModule

class Server(ServerModule):
    def __init__(self, args, sd, gpu_server):
        super(Server, self).__init__(args, sd, gpu_server)
        self.model = FedAux(self.args.n_feat, self.args.n_dims, self.args.n_clss, self.args.l1, self.args).cuda(self.gpu_id)
    ...
    def update(self, updated):
        ...
        for c_id in updated:
            ...
            local_auxes.append(self.sd[c_id]['aux'].copy())
        ...
        assert n_connected == len(local_auxes)
        ...
        for i in range(n_connected):
            for j in range(n_connected):
                ...
                # sim_matrix[i, j] = cosine_similarity(local_auxes[i].reshape(1, -1), local_auxes[j].reshape(1, -1))[0][0]
