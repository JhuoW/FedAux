...
class Client(ClientModule):
    ...
    def __init__(self, args, w_id, g_id, sd):
        ...
        self.model = GNNAUX(self.args.n_feat, self.args.n_dims, self.args.n_clss, self.args).cuda(g_id) 
    ...
    def train(self):
        ...
        for ep in range(self.args.n_eps):
            ...
            for _, batch in enumerate(self.loader.pa_loader):
                ...
                #################################################################
                for name, param in self.model.state_dict().items():
                    if 'aux' in name or 'conv' in name or 'clsif' in name:
    ...
    def transfer_to_server(self):
        # print(get_state_dict(self.model)['aux'])
        # print(get_state_dict(self.model)['aux'].shape)
        self.sd[self.client_id] = {
            'model': get_state_dict(self.model),
            'train_size': len(self.loader.partition),
            'aux': get_state_dict(self.model)['aux']
        }
